/**
 * RAGina Pro – Full Application (UI + Engine) – ENHANCED
 * Version 2.4.0 – Now with all features from ragina.js:
 *   loadFolder, loadData, getEngine, ask, __RAGINA_INDEX__,
 *   avatar, bubbleIcon, theme.primary, personality, sassy quotes.
 *
...
(function() {
    ...
    // ─── READ CONFIG ──────────────────────────────────────────────────────
    const CONFIG = { ...DEFAULT_CONFIG, ...(window.RAGINA_CONFIG || {}) };
    ...
    // ─── STYLES (with dynamic primary color & position) ────────────────────
    ...
    class RAGinaProUI {
        ...
        saveToLocalStorage() {
            try {
                ...
                document.getElementById('raginaChatStatus').textContent = 'Auto‑saved ✓';
                document.getElementById('raginaChatStatus').style.color = '#6C63FF';
            } catch (e) {
        ...
        injectHTML() {
            injectStyles();
            console.log('🔮 RAGina Pro: Injecting UI...');
            const primary = CONFIG.theme.primary || '#6C63FF';
            const avatarUrl = CONFIG.avatarUrl || 'https://ragina-crawler-ragina.vercel.app/ragina-logo.png';
            const bubbleContent = CONFIG.bubbleIcon || avatarUrl
                ? `<img src="${avatarUrl}" alt="RAGina" style="width:44px;height:44px;border-radius:50%;" onerror="this.parentElement.innerHTML='🔮'">`
                : '🔮';
            ...
            console.log('🎯 RAGina Pro UI ready.');
        }
        ...
        async speakText(text) {
            ...
            try {
                ...
                return new Promise(resolve => { this.currentAudio.onended = resolve; });
            } catch (e) {
    ...
    const RAGinaPro = {
        ...
        init(config) {
            ...
            // Then restore chat and load index from URL
            setTimeout(async () => {
        ...
        async loadFolder(fileList) {
            if (!this.ui) {
                console.warn('UI not initialized. Call init() first.');
                return;
            }
    ...
    const autoStart = () => {
        if (window.RAGINA_CONFIG) {
            RAGinaPro.init(window.RAGINA_CONFIG);
        } else if (window.__RAGINA_INDEX__) {
            RAGinaPro.init({ indexData: window.__RAGINA_INDEX__ });
        } else {
            RAGinaPro.init();
        }
    };
    ...
    // ─── EXPOSE ──────────────────────────────────────────────────────────────
    window.RAGina = RAGinaPro;
    window.RAGinaPro = RAGinaPro;
